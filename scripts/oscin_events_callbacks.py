...
_SONG_META_NUMERIC = ('key', 'duration', 'bpm')
...
def _enrich_fx_active(info: dict):
	...
	cache = parent.Djay.fetch('fxTypes', {})
...
def _clear_artwork(turntable: str):
	...
	mfi = parent.Djay.op(f'artwork_{turntable}')
	if mfi is not None:
...
#
# Passive liveness signal. oscin_unknown_callbacks stamps
# parent.Djay.store('connHeartbeat', absTime.seconds) on every inbound
# /djay/* message (its scope is *). This tick reads that timestamp and
# flips parent.Djay.par.Status accordingly.
#
# CRITICAL — DO NOT re-introduce any outbound OSC here. An earlier version
# sent /djay/request/turntable<N>/artwork as a probe to force a round-trip;
# that pattern triggered a ~14GB/30s memory leak in djay v4. The watchdog
# is now purely read-only on djay's side.
#
# Re-entrancy: generation counter on /djayPro lets a fresh module load
# supersede the previous chain instead of running two ticks in parallel
# (textDATs are externalized; every save reimports this module).
...
def _watchdog_tick(gen: int) -> None:
	...
	fresh = last is not None and (absTime.seconds - last) <= _STATUS_TIMEOUT_S
	_set_status('Active' if fresh else 'Disconnected')
	run('args[0](args[1])', _watchdog_tick, gen,
		delayMilliSeconds=int(_STATUS_TICK_S * 1000))
...
def onReceiveOSC(dat: oscinDAT, rowIndex: int, message: str,
	...
	if callback_name in _DEFERRED_RISING:
		defer_ms = int(float(parent.Djay.par.Messagedeferwindow.eval()) * 1000)
		if defer_ms > 0:
			_pending[key] = run(
